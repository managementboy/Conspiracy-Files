...
function UI.openHelp()
    safe(function()
        ...
        if generated() then UI.help.text="SURVIVE FIRST\nThese notes record documents you have inspected. They do not assign objectives.\n\nINSPECT\nTake a generated document into your inventory, then choose Inspect Investigation Document.\n\nJOURNAL AND EVIDENCE\nJournal records discovery order. Evidence lets you select and reread each document. Connections appear only between documents you have inspected.\n\nNAVIGATION\nUse the list and Journal/Evidence buttons. Tab and arrow keys navigate; Page Up/Down scroll. Contrast changes the reading colors. Use X or Close to dismiss." end
...
local function generatedRows(section)
    ...
    local case=wrapper and wrapper.canonical and wrapper.canonical.case
    ...
    for i,r in ipairs(known) do
        ...
        rows[i]={id=r.id,ordinal=i,title=section=="journal" and "Inspected "..r.title or r.title,
            summary="Document - Inspected - Discovery "..i,detailText=detail}
    end
    return rows
end
function Window:rows()
    if generated() then return generatedRows(self.section) end
        ...
        local ok,why=pcall(function()
            for _=1,256 do
                if i>=buildings:size() then
                    log("Audit complete: "..drawn.." drawn buildings, "..missing.." other footprints (first 60 detailed).")
                    stopAudit();return
                end
                ...
                if not b:isBasement() and cx>=area.minX and cx<=area.maxX and cy>=area.minY and cy<=area.maxY then
                    ...
                    if reason=="drawn" then drawn=drawn+1 else
                        ...
                        if missing<=60 then
                            ...
                            log("Audit "..x..","..y..".."..x2..","..y2.." id="..id.." reason="..reason.." address="..(assigned and assigned.label or "none").." rooms="..table.concat(names,","))
                        end
                    end
                end
...
local function session()
 local w=ModData.get("ConspiracyFiles.Generated.G2");return w and w.canonical
end
...
function M.before(character,item,source,destination,square)
 ...
 local c=session();local md=item and item:getModData();local a=c and md and c.assignments[md.cfGeneratedId]
...
function M.after(candidate,item)
 ...
 local c=session();local a=c and c.assignments[candidate.id]
 ...
 commit(next);log("Finding location recorded. Inspect the document to add it to your evidence.")
end
function M.update()
 if not allowed() or not getPlayer() then return end
 local c=session();if not c then return end
...
function M.note(id)
 local c=session();if not c or not known(c,id) then return nil end
...
function M.status()
 local c=session();local r=read();local written,pending,missing=0,0,0
 if c and r then for _,id in ipairs(c.known) do local v=r.records[id]
...
function M.drawRecords(ui,c,r)
 if not ui or not c or not r or not ui.mapAPI or ui.mapAPI:getZoomF()<14 then return end
 local groups,order={},{}
