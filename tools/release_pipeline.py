...
def production_files(config: dict) -> list[Path]:
    ...
    state = (MOD_ROOT / "common/media/lua/shared/ConspiracyFiles/ThreadState.lua").read_text(encoding="utf-8")
    schema_match = re.search(r"schemaVersion\s*=\s*([0-9]+)", state)
    if not schema_match or int(schema_match.group(1)) != config["state_schema_version"]:
        raise ReleaseError("release state_schema_version does not match ThreadState")
    return selected
