...
set -uo pipefail
# ONE FUNCTION, for the reason spelled out in campaign.sh: bash reads a script
# from disk incrementally, so an edit while this runs shifts the byte offset
# under the running shell and kills it mid-run. Demonstrated 2026-09-22 - an
# unwrapped script hit "line 5: 2: command not found" and restarted its output;
# the wrapped one ran to completion.
cf_main() {
