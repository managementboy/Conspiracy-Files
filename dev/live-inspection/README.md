...
An unattended profile must declare `interaction_scope=["startup-gate"]`, a non-T10 criterion list, the exact click gate pattern `game loading took`, the exact observer pattern `\[CF-INSPECT\].*kind=PLAYER_READY`, `max_actions=1`, `action="left-click"`, the exact supported `supported_game_version="42.20.4"`, and a positive post-signature settle time shorter than its at-most-30-second signature freshness budget. Missing, malformed, broad-prefix and other-version pins are rejected before a run. `Return`/`space` are rejected: the 2026-09-02 real-desktop replay showed a successful Return XTEST command without any PZ state transition.
