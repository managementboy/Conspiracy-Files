...
The generated observer attaches exact run, disposable save, observer-mod ID, random per-run session ID, monotonic event sequence, source timestamp, payload mode/ID/checksum, validated active-mod identity and the exact pinned game version to `PLAYER_READY`. The observer itself refuses an exact-version mismatch, and delivery correlation independently requires the same `42.20.4` value. Every field is required and duplicate, missing, malformed or foreign values are rejected.
