...
It is development tooling, not production Conspiracy-Files code. It does not change vanilla game files, install Java/JNI helpers, inject into the process, alter security settings, or expose normal-play diagnostics. This harness retains its startup-only boundary: P4-R48 permits one narrowly verified ordinary startup-gate action for non-T10 runs; the current evidence-supported implementation is one left click and does not permit T10, context-menu, inventory, gameplay or acceptance automation. ADR-0006's T10/E08 helper is a separate contract and does not weaken this harness.
...
Every profile must declare the exact supported `supported_game_version="42.20.4"` installed-build contract because the runner constructs runtime identity for both manual and unattended runs. Missing, malformed, broad-prefix and other-version pins are rejected before a run. In addition, an unattended profile must declare `interaction_scope=["startup-gate"]`, a non-T10 criterion list, the exact click gate pattern `game loading took`, the exact observer pattern `\[CF-INSPECT\].*kind=PLAYER_READY`, `max_actions=1`, `action="left-click"`, and a positive post-signature settle time shorter than its at-most-30-second signature freshness budget. `Return`/`space` are rejected: the 2026-09-02 real-desktop replay showed a successful Return XTEST command without any PZ state transition.

The loading signature is only the start of a bounded readiness check, not proof that PZ accepts input. After the settle interval, the runner focuses the sole mapped title-matching window owned by the launcher process group and captures a fresh full-frame PNG as evidence. Per the owner decision, PZ accepts a click anywhere inside the game client, so action admission no longer depends on label pixels, label position, or a fixed resolution. The runner records the actual client origin and dimensions, rejects zero/tiny/off-screen or ambiguous windows, and clicks the normalized client center with a two-pixel safety inset. The 1280x720 client is the intentional UI/UX design target; 960x1008 and 1920x1008 remain compatibility geometries, and no fixed resolution is a harness prerequisite.

After the screenshot and again after pointer warp immediately before XTEST, the runner revalidates launcher PID/start time, window PID/start time, sole-window identity, title, mapped state, client root origin/dimensions, active window, X11 focus, pointer root coordinates and topmost owned frame. It then establishes the exact console device/inode/EOF byte boundary, discards any partial boundary record, records the current observer sequence watermark, and atomically writes `startup-readiness-cursor.json` before the sole click. Only complete records whose first byte is at or after that cursor are eligible. A source `emittedAtMs` at or before action completion, at or before the persisted cursor boundary, more than the explicit 1,000 ms future-clock tolerance beyond observation/file time, a non-increasing sequence, or a duplicate/conflicting post-cursor record fails closed, so unread, partial, future-dated, replayed, reordered and late-flushed pre-action data cannot confirm delivery.

The generated observer attaches exact run, disposable save, observer-mod ID, random per-run session ID, monotonic event sequence, source timestamp, payload mode/ID/checksum, validated active-mod identity and the runtime `gameVersion` line to `PLAYER_READY`. The observer itself refuses a runtime mismatch at the coarse `42.20` precision reported by `getGameVersion()`, while the profile keeps the exact installed-build pin at `42.20.4`. Every field is required and duplicate, missing, malformed or foreign values are rejected.

Readiness-shaped records are parsed and journaled sequentially in exact byte-offset order. Each record is independently checked against every cursor, time/version, run/save/observer, payload, session, sequence and final X11 identity requirement before the one-shot policy is applied. A malformed, foreign or otherwise rejected record before the first valid candidate is retained immediately as `REJECTED` and does not consume the unique-valid slot, so a later unique valid record may confirm even when both arrived in one read. The first valid candidate remains uncommitted while the follower drains complete adjacent records through one quiet poll and waits out any partial record. A second independently valid candidate fails closed as a duplicate/conflict. The conservative post-candidate-noise policy also fails closed when any readiness-shaped record—valid or rejected—follows the first valid candidate during that drain; every record from the candidate onward is retained as rejected evidence. This ordering is identical for one buffer, records split across reads and one record per poll, and no intermediate `CONFIRMED` state is written.

Immediately before confirmation, one bounded X11 snapshot re-reads `DISPLAY`, launcher/window PID start identities, process group, sole exact XID/title/origin/dimensions, active window, input focus, the pointer's exact normalized client-center root coordinates and the topmost pointer child/owned frame. The process/window/focus tuple is read again around the pointer query to close that observation interval, including before release of the held button. Pointer movement, an overlay/foreign topmost child, focus/active drift, process reuse, unsafe geometry or geometry/session drift fails closed and cannot emit another input.

Every matching readiness record is retained in immutable order in the fsync-backed append-only `startup-readiness-evidence.jsonl` and mirrored in the atomically replaced `unattended-startup-input.json`. Each journal row includes Base64 byte-exact content with its original LF/CRLF terminator and SHA-256, decoded display text and UTF-8 status, exact byte offsets and log identity, file/source/observation timestamps, parse result, accepted/rejected classification and rejection reason. Duplicate/conflicting rows are all retained. A rejected row records `NOT_CONFIRMED`; a later unique valid row may confirm without changing any earlier journal row. Journal append is fsynced before summary-state replacement, so an interruption cannot make already-written record evidence disappear. XTEST completion alone is never delivery success.

Every unattended bundle writes `criteria-disposition.json` with T10 and CF-V01-E08 as `NOT RUN`. A profile that requests T10/E08, right-click, context menu, inventory/menu, gameplay or acceptance interaction is refused before save, mod or control mutation; the refused run still retains that disposition and a cleanup manifest. ADR-0006 is deliberately outside this reusable harness and does not change this refusal. There is no retry input budget.
...
They cover profile validation, real-save marker enforcement, exact control round trips (including absent files), exclusive locking, renderer classification, gate/process diagnostics, PID/start-time/window ownership filtering, normalized action points across supported window sizes and origins, unsafe geometry and focus/window races, one-shot/stale-signature input bounds, stale log and screenshot rejection, transition-confirmed delivery, T10/E08 and interaction-scope refusal, production payload validation/install/recovery cleanup, sanitization, Lua/Python parsing, and static rejection of embedded P2/R2 coordinates, Xephyr, forced software mode, right-click automation, and the prohibited helper name in reusable core.
...
The dedicated `cf-v01-e08-owner-attended.toml` profile is retained as historical manual evidence. It first installs the disposable T10 context-menu probe alongside the inspection probe. `OWNER_PHASE_READY|status=HOLD` is emitted only after the T10 probe reports its exact six-item inventory plus one Ground fixture, registered handlers, and a zombie-free player square. Missing/duplicate fixtures, absent probe/handlers, unavailable safety observation, or a zombie appearing during HOLD fail closed before owner handoff. All fixtures and the probe copy live only in the disposable run and are archived by cleanup. The 7200-second hold is bounded by the validator’s 86400-second maximum; timeout, malformed release, signal, or process death fails closed. This harness keeps T10/E08 owner-only; ADR-0006's separate contract must not be routed through it. Unrelated repeated engine animation warnings are retained in sanitized evidence and never treated as readiness.
