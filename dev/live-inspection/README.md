...
Immediately before confirmation, one bounded X11 snapshot re-reads `DISPLAY`, launcher/window PID start identities, process group, sole exact XID/title/origin, 960x1008 geometry, active window, input focus, the pointer's exact authorized lower-center root coordinates and the topmost pointer child/owned frame. The process/window/focus tuple is read again around the pointer query to close that observation interval. Pointer movement, an overlay/foreign topmost child, focus/active drift, process reuse or geometry/session drift fails closed and cannot emit another input.
