...
The loading signature is only the start of a bounded readiness check, not proof that PZ accepts input. After the settle interval, the runner focuses the sole mapped title-matching window owned by the launcher process group and captures a fresh full-frame PNG as evidence. Per the owner decision, PZ accepts a click anywhere inside the game client, so action admission no longer depends on label pixels, label position, or a fixed resolution. The runner records the actual client origin and dimensions, rejects zero/tiny/off-screen or ambiguous windows, and clicks the normalized client center with a two-pixel safety inset. The 960x1008 client remains the intentional UI/UX design target, but is not a harness prerequisite.
