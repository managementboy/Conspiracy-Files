...
function getFileReader(path) local file = io.open(path, "r"); if not file then return nil end; return { readLine=function() return file:read("*l") end, close=function() file:close() end } end
...
if mode == "hold" then for i=1,3 do for _, fn in ipairs(callbacks.OnTick) do fn() end end; assert(quitCount == 0); realPrint("ASSERT HOLD") else
  if mode == "timeout" then for i=1,61 do for _, fn in ipairs(callbacks.OnTick) do fn() end end; assert(quitCount == 1); realPrint("ASSERT SAFE") else
    if mode == "release" then releaseContent=validRelease
    elseif mode == "future" then releaseContent="CF_OWNER_RELEASE|version=1|status=RELEASED|gate=player-ready-modal-check|run_id=RUN|observer_id=OBS|session_id=SESSION|nonce=N1|ready_sequence=3|ready_at_ms=1000|released_at_ms=1001"
    ...
    if mode == "partial" then releaseContent=validRelease; publish(releaseContent, true); for _, fn in ipairs(callbacks.OnTick) do fn() end end
