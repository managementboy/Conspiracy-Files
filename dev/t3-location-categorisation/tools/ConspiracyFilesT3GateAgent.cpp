...
static DWORD WINAPI gateWorker(LPVOID) {
    ...
    if (g_vm->AttachCurrentThread(reinterpret_cast<void**>(&env), nullptr) != JNI_OK) {
        std::fprintf(stderr, "[CF-T3-AGENT]|ERROR|AttachCurrentThread failed\n");
        std::fflush(stderr);
        return 1;
    }
        ...
        if (stateMachine != nullptr) {
            ...
            if (currentId != nullptr) {
                ...
                if (loadingClass != nullptr && current != nullptr && env->IsInstanceOf(current, loadingClass)) {
                    ...
                    if (doneId != nullptr && forceDoneId != nullptr && env->GetStaticBooleanField(loadingClass, doneId) && armed) {
                        env->SetBooleanField(current, forceDoneId, JNI_TRUE);
                        armed = false;
                        std::printf("[CF-T3-AGENT]|GATE_RELEASED|reason=engine-done-true\n");
                        std::fflush(stdout);
                    }
                } else {
                    armed = true;
...
extern "C" JNIEXPORT jint JNICALL Agent_OnLoad(JavaVM* vm, char*, void*) {
    ...
    CloseHandle(thread);
    std::printf("[CF-T3-AGENT]|STARTED|mode=loading-gate-only\n");
    std::fflush(stdout);
    return JNI_OK;
}
