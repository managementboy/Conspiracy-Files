...
local scheduled = nil
local tickCount = 0
local matrixFinished = false
local cases = {}
local domain = { inspectCalls = 0, markCalls = 0, marked = {}, faultLogs = 0 }
...
local function domainMarkInteresting(playerNum, subject)
    ...
    local key = subject.token or subject.assetId
    local created = domain.marked[key] ~= true
    if created then domain.marked[key] = true end
...
local function addSubjectActions(playerNum, context, subjects, source, test)
    ...
    local marked = domain.marked[subject.token or subject.assetId] == true
    ...
    logEvent("MENU_ADD", {
        ...
        "invalidAlsoSelected="..tostring(#invalidReasons),
    })
...
local function removeOldItems()
    ...
    for i=0,items:size()-1 do
        ...
        if tostring(item:getName()):match("^T10 ") then remove[#remove+1] = item end
    end
...
local function createCases()
    removeOldItems()
    cases.revealed = stamp(instanceItem("Base.Note"), "revealed-note", true, true, false)
    cases.key = stamp(instanceItem("Base.Key1"), "key-b37", true, true, false)
    cases.hidden = stamp(instanceItem("Base.Note"), "hidden-note", false, true, false)
    cases.invalid = stamp(instanceItem("Base.Paperclip"), "invalid", true, false, false)
    cases.fault = stamp(instanceItem("Base.LetterHandwritten"), "fault", true, true, true)
    getPlayer():getInventory():AddItem(cases.revealed)
    getPlayer():getInventory():AddItem(cases.key)
    getPlayer():getInventory():AddItem(cases.hidden)
    getPlayer():getInventory():AddItem(cases.invalid)
    getPlayer():getInventory():AddItem(cases.fault)
    local unownedItem = stamp(instanceItem("Base.Photo"), "unowned-photo", true, true, false)
    cases.unownedWorld = getPlayer():getCurrentSquare():AddWorldInventoryItem(unownedItem, 0.5, 0.5, 0)
    cases.unowned = unownedItem
    logEvent("CASES_CREATED", {"inventory=5", "world=1"})
end
...
local function invoke(option)
    if not option or not option.onSelect then return false end
    option.onSelect(option.target, option.param1, option.param2)
    return true
end

local function runCase(name, items, expectedInspect, expectedMark, activate)
    ...
    if activate then invoke(optionByKey(context, activate)) end
    return pass
end
...
local function runMatrix()
    ...
    local pass = true
    pass = runCase("raw-single-revealed", {cases.revealed}, 1, 1, INSPECT_KEY) and pass
    ...
    pass = runCase("mark-activation", {cases.key}, 1, 1, MARK_KEY) and pass
    local markedContext = newMockContext()
    inventoryHandler(0, markedContext, {cases.key})
    local markedOption = optionByKey(markedContext, MARK_KEY)
    local markedPass = markedOption and markedOption.notAvailable == true and markedOption.onSelect == nil
    logEvent("MATRIX_CASE", {"case=already-marked", "pass="..bool(markedPass), "options="..describe(markedContext)})
    pass = markedPass and pass
    local faultContext = newMockContext()
    inventoryHandler(0, faultContext, {cases.fault})
    local before = domain.inspectCalls
    invoke(optionByKey(faultContext, INSPECT_KEY))
    local faultPass = domain.inspectCalls == before and domain.faultLogs == 1
    logEvent("MATRIX_CASE", {"case=exception-containment", "pass="..bool(faultPass), "inspectCalls="..domain.inspectCalls, "faultLogs="..domain.faultLogs})
    pass = faultPass and pass
    ...
    local realOk, realErr = pcall(function()
        local first = ISInventoryPaneContextMenu.createMenu(0, true, {cases.revealed}, 500, 300, nil)
        local second = ISInventoryPaneContextMenu.createMenu(0, true, {cases.revealed}, 500, 300, nil)
        logEvent("REAL_INVENTORY_MENU", {
            "firstInspect="..tostring(first and countKey(first, INSPECT_KEY) or -1),
            "secondInspect="..tostring(second and countKey(second, INSPECT_KEY) or -1),
            "options="..(second and describe(second) or "<nil>"),
        })
    end)
    logEvent("REAL_MENU_CALL", {"ok="..bool(realOk), "error="..safe(realErr)})
    pass = realOk and pass
    matrixFinished = true
    logEvent("MATRIX_SUMMARY", {
        "status="..(pass and "PASS" or "FAIL"),
        "inspectCalls="..tostring(domain.inspectCalls),
        "markIntentCalls="..tostring(domain.markCalls),
        "markedEvidence="..tostring(domain.marked["cf-t10-token:key-b37"] and 1 or 0),
        "faultLogs="..tostring(domain.faultLogs),
    })
    local inventory = getPlayerInventory(0)
    if inventory then inventory:setVisible(true); inventory:setPinned() end
    logEvent("READY_FOR_UI", {"instruction=right-click-T10-items; F6-inspect; F7-mark; F8-world-menu"})
end
...
local function onKeyPressed(key)
    if not active or not matrixFinished then return end
    if key == Keyboard.KEY_F6 then
        local context = newMockContext(); inventoryHandler(0, context, {cases.revealed}); invoke(optionByKey(context, INSPECT_KEY))
    elseif key == Keyboard.KEY_F7 then
        local context = newMockContext(); inventoryHandler(0, context, {cases.revealed}); invoke(optionByKey(context, MARK_KEY))
    elseif key == Keyboard.KEY_F8 and cases.unownedWorld then
        local ok, err = pcall(ISWorldObjectContextMenu.createMenu, 0, {cases.unownedWorld}, 500, 300, false)
        logEvent("REAL_WORLD_MENU", {"ok="..bool(ok), "error="..safe(err)})
    end
end
