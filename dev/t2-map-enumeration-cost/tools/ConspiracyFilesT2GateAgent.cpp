...
static DWORD WINAPI gateWorker(LPVOID) {
    ...
    // event at the final loading gate. Wait until the engine reports loading
    // complete, then release only that gate.
    Sleep(5000);
    JNIEnv* env = nullptr;
    if (g_vm->AttachCurrentThread(reinterpret_cast<void**>(&env), nullptr) != JNI_OK) {
        std::fprintf(stderr, "[CF-T2-AGENT]|ERROR|AttachCurrentThread failed\n");
        std::fflush(stderr);
        return 1;
    }
            ...
            if (currentId != nullptr) {
                ...
                if (loadingClass != nullptr && current != nullptr && env->IsInstanceOf(current, loadingClass)) {
                    ...
                    if (doneId != nullptr && forceDoneId != nullptr
                        ...
                        armed = false;
                        std::printf("[CF-T2-AGENT]|GATE_RELEASED|reason=engine-done-true\n");
                        std::fflush(stdout);
                    }
                } else {
                    armed = true;
...
extern "C" JNIEXPORT jint JNICALL Agent_OnLoad(JavaVM* vm, char*, void*) {
    ...
    if (thread == nullptr) {
        std::fprintf(stderr, "[CF-T2-AGENT]|ERROR|CreateThread failed\n");
        std::fflush(stderr);
        return JNI_ERR;
    }
    CloseHandle(thread);
    std::printf("[CF-T2-AGENT]|STARTED|mode=loading-gate-only\n");
    std::fflush(stdout);
    return JNI_OK;
}
