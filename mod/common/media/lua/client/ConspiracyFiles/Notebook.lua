...
-- The projection itself is ConspiracyFiles/EvidenceRows; `generated` stays
-- here because deciding what counts as a live runtime is this module's job.
local function generatedRows(section) return EvidenceRows.build(section,generated) end
...
function Window:refresh(preferred)
    ...
    if #rows==0 then
        ...
        self.document:setDocument("Inspect an unusual document or mark an acquired object worth remembering. These records hold encounters; they do not assign objectives.",UI.highContrast)
        self:layout(); return
    end
