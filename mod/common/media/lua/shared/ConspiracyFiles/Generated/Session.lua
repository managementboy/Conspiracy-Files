...
function S.validate(root)
    ...
    for _,d in ipairs(root.case.documents) do
        ...
        if not fields(a,{physicalToken=true,target=true,status=true}) or a.physicalToken~="cf-g2:"..d.id
            or not ({pending=true,placing=true,placed=true,unknown=true,conflict=true})[a.status]
            or not S.target(a.target,sites[d.locationId]) then return false,"invalid assignment" end
    end
...
function S.create(case,targets,documentTargets)
    ...
    for _,d in ipairs(case.documents) do root.assignments[d.id]={physicalToken="cf-g2:"..d.id,target=copy(documentTargets and documentTargets[d.id] or targets[d.locationId]),status="pending"} end
