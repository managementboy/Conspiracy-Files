...
function WorldRuntime.new(options)
    ...
    function api.start()
        ...
        for _, assetId in ipairs(Content.thread.documentAssetIds) do enqueuePlacement(assetId) end
    end
    ...
    function api.onTick()
        ...
        identityIndex = (identityIndex % #Content.thread.documentAssetIds) + 1
        local assetId = Content.thread.documentAssetIds[identityIndex]
