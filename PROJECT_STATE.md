...
- **T5 persistent physical item identity:** complete as historical mechanism evidence. A fixed multi-load Build 42.20.4 matrix carried one detached-prestamped `Base.Note` through inventory, ordinary container, floor, vehicle and back to inventory with the same ModData token and observed engine ID, then proved permanent removal stayed absent. Real disposable-character death moved one stamped item to the corpse. `copyModData`/`CopyModData` produced distinct engine items with the same token, so duplicates are a sticky `conflict`; engine IDs remain diagnostics only. T5 did not prove the later active Asset/token-pair invariant; P4-R37 and the checked-in schema-2 live matrix govern production. See `docs/research/T5_PHYSICAL_ITEM_IDENTITY.md`.
...
- **T10 cooperative Inspect integration:** complete through the historical P4-R44 manual-GUI route. Repeated inventory-pane menus preserved vanilla actions and another additive listener, privately keyed Inspect activated once, Mark Interesting emitted one intent and stayed disabled across reload, hidden/invalid/ambiguous/unowned states behaved conservatively, and injected faults were contained without a crash. Ground inventory is the supported dropped-item surface; direct world right-click received zero inventory subjects and is explicitly unsupported. Controller activation was unavailable. ADR-0006 adds only an unrun, fail-closed offline contract for a checksum-pinned ZombieBuddy helper against the exact Build 42.20.4 disposable T10/E08 fixture; it creates no acceptance claim. See `docs/research/T10_COOPERATIVE_INSPECT.md` and `docs/decisions/ADR-0006-t10-e08-bounded-zombiebuddy-helper.md`.
...
- **v0.1 production integration candidate:** the Build 42 package/bootstrap, fail-closed multiplayer gate, additive lifecycle hooks, bounded scheduler, error budgets, staged persistence, placement, physical identity, item reader/actions, arrival, notebook/evidence/help UI, supported death/reload checkpoints and deterministic packager are implemented and tested offline. The schema-2 integrity work adds exact per-kind journal replay, one shared active Asset/token-pair gateway, legacy-flat quarantine, transactional registration generations, unreadable authored-carrier collision handling and fail-closed range selection with normal-suite adversarial coverage. CF-V01-E02–E14 still require their named live Build 42 acceptance matrices; the current corrective candidate requires fresh independent offline QA and is not accepted or released.
...
- `dev/t10-zombiebuddy-helper/` is a separate repository-contained offline contract and test suite. It does not contain a ZombieBuddy binary/configuration or modify the live-inspection harness's startup-only authority. Any actual helper candidate remains pending independent QA and a separately reviewed disposable live matrix.
...
The next finishable milestone is PM-GOV-001 independent offline QA of the exact current schema-2 corrective candidate. Only after that gate passes should work resume on the checked-in live E02–E14 matrices for the supported Build 42 line, including `docs/testing/SCHEMA2_PAIR_IDENTITY_LIVE_MATRIX.md`. If owner separately requests the T10/E08 helper matrix, first independently QA the exact ADR-0006 helper contract and separately review a pinned helper adapter; no helper evidence substitutes for the named live criterion. Any production-code change invalidates that QA. No release, distribution or owner acceptance is implied by the offline implementation.
