...
**DR-20260924-SEARCHED-MEANS-LOOKED — "already searched" is the player having looked, never the engine having generated loot.** Fitness Instructor first-mystery audit, real game, 2026-09-24 (`20260924T183959`, source `0d86a1b`). Standing outside a Muldraugh house the survivor had never entered (Building 10675,10266), 23 of its 24 indexed fixed containers answered `isExplored()=true`, `isHasBeenLooted()=false`, with loot inside. The engine sets `explored` when it generates loot, and it does that for a whole building as the chunk loads — before the survivor can reach any of it. Every guard that read `isExplored` as "the player searched this" (`FixedContainerRuntime.resolve/fresh`, `Storage.scan`, `Carriers`) therefore refused nearly every container a survivor could ever walk up to. Consequences measured the same day: the filler's instalments reported "no-containers" at their own sites, every indexed plan was unplanned the moment its building loaded, and the three object clues of the opening case were still waiting after the survivor stood at their site.

**Decided:** "searched" is either of two signals, and nothing else — `isHasBeenLooted()` (the engine sets it when the player takes something out; opening does not set it, measured live) or the mod's own mark `cfSearched` on the container's parent object, written the moment `ISInventoryPage:selectContainer` shows that container's contents (`SearchedContainerWatch`). `isExplored` is not consulted for this anywhere. The rule itself is unchanged: nothing is inserted into a place the player already looked into, and a container whose state cannot be read is still refused.

**Also fixed under the same audit:** the filler took `waiting[1]` every attempt, so one clue with nowhere to go blocked every clue behind it for three in-game days (`Session.pick`, a wrapping cursor; one clue per attempt is kept). Tests: `test/searched_means_the_player_looked.lua`, `test/fixed_container_runtime.lua`, `test/case_instalments.lua`; both defects are in the mutation corpus as 006 and 007. Native evidence: `docs/management/evidence/linux-autotest/*-fitness-mystery-audit.txt`.


**DR-20260924-SECOND-CASE-OPEN-ORDER — a later case may name a site whose chunk is not loaded, exactly as the opening may.** Native suite 2026-09-24 (`core_loop 20260924T223914`: "no second case within 150 s with the gap removed: busy x0"). Case preparation asked `World.resolve` about every selected target before committing a case; an indexed fixed-container signature for a building out of sight answers "unloaded", and that was read as `busy`. The opening branch had already been given the tolerance on 2026-09-23 (`4580e18`: "the partner may remain an indexed open order"); the ordinary branch had not, so once the opening was in place every second case whose sites were out of sight was refused. **Decided:** both branches apply one rule — only a live, non-indexed target must resolve now; an indexed signature is an open order the filler completes when the survivor arrives (P4-R133). `test/second_case_tolerates_indexed_partner.lua` holds the two branches to the same words.

Recorded with it, from the same suite run: the native checks written before the 2026-09-23 opening (the first clue a key delivered to the hand, the rest in instalments) and before DR-20260919-CONTINUITY (a thread outranks the closing answers) asserted the older behaviour and had not been run since. They now play the opening as designed — `settle_doc` stands at a waiting clue's site, `wait_furniture_clue` brings instalments in, `CFLoop.find` looks in the hand — and `core_loop` asserts that the second case follows a finding while the answers wait for the case after. No product behaviour was changed to satisfy a check; each check change names the run that showed it wrong.


**DR-20260925-OCCUPATION-OPENINGS — every Build 42 occupation starts its own mystery.** Owner, 2026-09-25: "expand the starting mysteries for all occupations", with the follow-up "how do we make sure we have a different start for each occupation each time?" Until now only the Fitness Instructor had an opening family (ten starts, DR-20260919-Q03's first slice); the other twenty-four occupations drew the generic personal opening. Now each has a family of its own (`Generated/OccupationOpeningScenarios.lua`, three starts each; the Fitness ten unchanged), on the five-finding grammar the Fitness starts proved in play: a thing in the pocket, an ordinary appointment in the survivor's name at the starting address, a farm- or cordon-related object in the wrong room, a spent hoard of more than one person's kit, and a transport scene that waits for a real vehicle. The occupation explains why this survivor could be sent to a house; it never explains immunity, invents rank, family or a crime, or names the central question (the plan's §1 boundary, `OCCUPATION_MYSTERIES_LINUX_PLAN_2026-09-19.md`; held by `test/occupation_openings.lua`).

**Both hidden pairs are met.** Ten families pin the farm pair (feed sacks, spent masks, a cooler) and fifteen the cordon pair (sandbags, radios, issued kit, a sealed case), so a campaign does not tell one story twenty-five times. Seven families (doctor, nurse, fire officer, repairman, security guard, and the Fitness ten) put a real key to the starting building in the survivor's hand before the case attaches; the rest deliver a tool or token of the trade when it does - a scorched screwdriver, a stained hand scythe, a badge with somebody else's number. Every pocket object is one an object rule would allow.

**The randomiser, answered.** A new game draws its seed once (`ZombRand`, `R.start`), and the family's start is `(seed-1) % variants + 1` - uniform over the family's authored starts, never consuming the case generator's own PRNG, pinned in the save so a reload rebuilds the same start. So with three starts a survivor of the same occupation meets a repeat one game in three; the Fitness ten, one in ten. Nothing persists across saves (the game offers no store outside a save that is not sandbox settings), so "different every time" is a matter of how many starts a family has, not of memory. Adding a start to a family is one table row; the tests reach every start of every family by seed.

Not decided here: how much biography an opening may establish (the plan's open boundary stands; these families establish an errand and nothing more), and whether occupations should later share starts with neighbouring trades (a chef's start for a burger flipper) when their own are exhausted.


**DR-20260925-OPENING-MEMORY — the randomiser remembers what this install has played, and does not prioritise it.** Owner, 2026-09-25: "we need to implement the randomizer that also remembers per game install what initiation clues we have used in the past. Those should not be prioritised." A save knows only itself, so the start of a new game was `(seed-1) % starts + 1`, blind to earlier games: nurse played 1, 1, 2 across three fresh saves (`20260925T191517`). **Decided:** one small text file beside the saves - `Zomboid/Lua/ConspiracyFiles_openings.txt`, one line per family, `profession<TAB>start:count,...` - counts the starts this install has played (`OpeningMemory`, pure; `OpeningMemoryStore`, the file). A first case chooses the **least-played** start of its family; the seed decides only between equals, so a check can say why a start was chosen. The count is written only after the case is committed, never for a game that failed to build. Missing, unreadable or corrupt, the file costs a game nothing: an empty memory is the first game on an install. Measured on Linux (`profession_openings nurse 4`, `20260925T195330`): 3, 2, 1, then 3 - every start once before any repeat, each choice held to the memory read before its case was built.

Not decided here: sharing the memory across machines (Steam does not sync `Zomboid/Lua`), or letting the owner reset it from the device; `rm` on the file is the reset today.


**DR-20260925-SCENE-AT-COMPLETION — a transport scene reality never promoted does not hold a case open.** Live test of the occupation openings, 2026-09-25 (`core loop 20260925T200914`, the unemployed opening): every essential clue found, and the case did not complete, because its fifth clue - the transport scene, which waits for a *confirmed* vanilla vehicle near the second site - had nowhere to go. P4-R133 rightly refuses completion while a clue is unwritten, and P4-R133's expiry would have dropped it after three in-game days; the opening families' own text says the scene "cannot make the case fail merely because this save has no suitable nearby scene". It did not fail the case; it held "What do I make of it?" back for three days in every save without a matching scene, which the Fitness audits had all seen and recorded as "not exercised". **Decided:** when every other clue is accounted for, a vehicle-intent clue still waiting is set aside at completion - dropped through the session's own drop, logged `no-scene-at-completion`, listed by `Session.gaps` among the clues the case never had, never silently - and the case completes on the clues it got. A transport scene that *was* placed must still be found like any clue. `test/vehicle_scene_does_not_hold_the_case.lua`.


**DR-20260925-FOLLOW-UP-KEEPS-COMPANY — the continuation keeps the company it follows.** Live test 2026-09-25 (`core loop 20260925T204628`, the unemployed opening; the console said `no case: continuation company mismatch`). The follow-up case ("still filing") writes `{ORG}` in its own papers and was handed a placeholder company (McCoy Logging) by the scenario accessor, while the thread it followed carried the opening's own company - Knox Bank, Sure Fitness, Al's Auto Shop. The generator refused the mismatch, the refusal was deferred as `no-containers`, and the retry hit the same refusal: after *any* profession opening, including the Fitness ten, no second case ever came. No check had followed a profession opening past its first case. **Decided:** a follow-up adopts the company of the thread it follows, and the thread now carries the company's grounding so the continuation stays as real as the case it continues (`grounding` is optional on a thread; threads saved before today still build). The refusal is gone from the generator. `test/profession_follow_up.lua` follows the Fitness, unemployed, electrician and nurse openings; native `core loop 20260925T214359` PASS - the second case follows the first's finding, company kept.


**DR-20260925-OCCUPATION-OPENINGS-WITHDRAWN — every mystery is different by design; the occupation families are withdrawn from routing.** Owner, 2026-09-25, on the twenty-four families built that afternoon (DR-20260925-OCCUPATION-OPENINGS): "Repetitions break the illusion of a true mystery. Every mystery has to be different by design. No one can be like the other." The families were the Fitness five-finding shape parameterised per trade - a thing in the pocket, a card in the survivor's name, a wrong-room object, a spent hoard, a transport scene - with the errand, company and object changed; after a second play the shape is the answer, and the memory of played starts (DR-20260925-OPENING-MEMORY) rotates variants, not shapes. **Decided:** (1) no profession but the Fitness Instructor routes to an opening family; the twenty-four families stay on disk as authored material (errands, companies, trade objects) and register nothing (`Occupations.ROUTED=false`); (2) the engine is to be rebuilt so that each starting mystery is its own design, with its own shape, count, mechanics and way of ending - the generator keeps placement, discovery, the pair binding and the record, and stops dictating what a mystery looks like. The Fitness ten are the same shape ten times and are the first candidate for redesign, not the template. Tests that pinned the shape ("five documents", "document 2 is a dated receipt", "document 5 waits for a vehicle") pinned the defect; what a test may hold is honesty and reality, never shape. Plan: `docs/design/EVERY_MYSTERY_ITS_OWN_2026-09-25.md`.


**DR-20260925-MYSTERY-BOUNDARIES — the owner's answers to the rebuild's six questions.** Owner, 2026-09-25, answering `EVERY_MYSTERY_ITS_OWN_2026-09-25.md` §7:

1. **A start may diverge from "an errand in my name"** - it may be about the person next door, a place, an event the survivor stumbles into - **but it must include the player's occupation** in it. The occupation is the one personal fact every start carries; the survivor's name, key or appointment are no longer required.
2. **A mystery may end without completing.** A question the survivor carries for the whole game is a legitimate ending. How THREADS shows it honestly (not "put down", not a state that implies "unsolved") is design work in the first such mystery.
3. **Nothing has to be in the pocket.** The first finding may be found - a drawer, a note, a door - with nothing on the survivor at the start.
4. **As many sites as needed**, of whatever kind the mystery needs; two buildings is no longer the rule.
5. **Mechanics need not be tested first.** A mystery may be designed around a tool/skill interaction before that interaction is proven on Linux; verification follows the design.
6. **The Fitness ten are included in the redesign** - not retired, not kept as they are.

And one question the owner adds to the rebuild's own list: **why is there a 240-character cap on an object's record body?** (`EvidenceKinds.OBJECT_MAX_CHARS`; the known reasoning is in PM_HANDOFF.md.) To be answered by the rebuild rather than inherited by it.
