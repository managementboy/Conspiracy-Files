...
| P4-R44 | **T10 may resume only through a manual-GUI session.** No helper/injected agent, quarantine restoration, antivirus exclusion or bypass, or alternate injection is permitted. The project owner manually launches and enters the disposable save and performs the requested right-clicks while the probe only logs menu callbacks and assertions. | Resumes the open cooperative context-menu test without reintroducing the disallowed automation/injection path. |
