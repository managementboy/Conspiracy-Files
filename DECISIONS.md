...
| P4-R36 | **Deferred placement uses `LoadGridsquare` only to enqueue relevant curated bindings plus an `OnGameStart` catch-up; reconciliation scans the exact target for a deterministic item stamp before any add. Stage `placing` under P4-R32, create and stamp the item while detached, add that exact instance, verify count one, then stage `placed`. One stamp repairs stale intent; zero after `placed` becomes `lost` with no blind respawn; more than one becomes `conflict`. Terminal pre-placement target loss becomes `unavailable`; mere unloading remains pending.** | T4 on Build 42.20.4 proved seven valid interruption paths stayed at one item through save/reload and a true stream-out/in. No Lua-visible atomic transaction across chunk/container files and Global ModData was found or tested, so ambiguous partial persistence must bias toward loss/fallback rather than duplication. See `docs/research/T4_EXACT_ONCE_PLACEMENT.md`. |
