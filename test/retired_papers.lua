-- A finished case's papers say so (P4-R118). Owner, 2026-09-15: two papers of a
...
-- right-click menu says they are already in the organiser. A paper from no
-- case gets neither, and a live case's paper keeps Inspect.
...
-- The menu, driven with a runtime that knows one live and one retired paper.
...
assert(#c.options==1,'a retired case paper must get exactly one option, got '..#c.options)
...
assert(#c.options==0,'a paper from no case must get no option, got '..#c.options)
c=menu(); Menu.fill(0,c,{paper(nil)})
assert(#c.options==0,'an ordinary item must get no option')

c=menu(); Menu.fill(0,c,{paper('doc-live')})
assert(c.options[1] and c.options[1].name=='Inspect Investigation Evidence','a live paper must keep Inspect')
...
assert(restamp and restamp:find('setDisplayCategory(categoryOf(md.cfGeneratedId))',1,true),
    'reloading must not stamp every paper as Evidence unconditionally')
...
assert(scan and scan:find('setDisplayCategory("EvidenceOld")',1,true),
    'the last-seen scan must mark retired papers Old, covering reloads and older saves')
...
assert(done and done:find('setDisplayCategory(categoryOf(md.cfGeneratedId))',1,true),
    'the paper in hand must be marked Old when its case retires')
