-- Inspecting a paper that is already in the notebook says nothing new
-- (audit follow-up, 2026-09-15).
--
-- PlayerVoice's once-per-thing memory lives only while the game runs, and
-- Session.inspect quietly returns true for a paper already known. So after a
-- reload, inspecting a paper noted in an earlier session again reached the
...
-- remembers whether the paper was known before this inspection and stays quiet
-- if it was.
...
assert(already and recorded and already<recorded,"R.inspect must decide whether the paper was already known BEFORE recording it")
assert(inspect:find("if voice and voice.onConnection and not already then",1,true),
    "the connection and pile lines must stay quiet for a paper already known")
print("PASS inspecting a paper already in the notebook announces nothing, even after a reload")
