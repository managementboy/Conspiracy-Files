...
local s=K.new();for _,f in ipairs(F)do s=add(s,f)end;local r,why=K.connections(s);assert(why=="complete"and #r==1 and r[1].statement:find("connection")and r[1].statement:find("unknown"));local q=K.new();for i=#F,1,-1 do q=add(q,F[i])end;assert(assert(K.connections(q))[1].id==r[1].id);local d,e=K.observe(s,F[1]);assert(e=="duplicate"and #assert(K.connections(d))==1);assert(K.observe(s,{kind="keySource",id="key",sourceToken="S",keyToken="x",keyId=7})==nil);assert(K.observe(s,{kind="keySource",id="wrong",sourceToken="S",keyToken="physical-2",keyId=7})and #assert(K.connections(s))==1)
