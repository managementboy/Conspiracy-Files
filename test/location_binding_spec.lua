...
test("CF-V01-E01 selected live bindings remain exact and evidence-backed", function()
    ...
    assertContainer(ids.key, { x = 13207, y = 3087, z = 0, room = "policeoffice", objectIndex = 2, containerIndex = 0, containerType = "filingcabinet", sprite = "location_business_office_generic_01_32" })
    assertEqual("Base.Key1", CF.Content.assets[ids.key].pzItemType)
    assertFalse(CF.Content.assets[ids.key].autoRecordEvidence)
