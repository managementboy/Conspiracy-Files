...
-- With no campaign loaded it must decline rather than throw. This is not a
-- footnote: both callers are scheduler jobs, so an unguarded ipairs(nil) here
-- would take down the job that was fixing the stall. The first version of the
-- fix did exactly that, and test/carrier_timer.lua caught it.
...
assert(R.retireIfAccounted(ready)==false,
    "no campaign loaded: it declines quietly instead of throwing inside a scheduler job")

-- WHAT THIS FILE DOES NOT PROVE, said plainly rather than asserted around.
-- Retiring for real needs a loaded wrapper with a matching case, which is a
-- game-side state this tier cannot build. Two other files carry that weight:
--   * test/carrier_timer.lua drives the REAL carrier watcher, and its
--     dropMissing path really does reach retireIfAccounted - that is how the
--     missing nil-wrapper guard above was found;
--   * test/case_completion_state.lua proves a case retired with a gap keeps its
--     state, its gap ids and its drop-path history through retirement, the deep
--     archive and the save wrapper.
-- What remains untested anywhere is the whole sequence in a running game:
-- inspect everything available, wait out three in-game days, and see the slot
-- freed. That is a tier-3 check and needs an injectable clock, because 72
-- in-game hours are unreachable in a real-time run - the reason the 77-minute
-- campaign run could not answer this question at all.

print("PASS retire after final drop: the runtime exposes the check, declines when it should, and never throws")
