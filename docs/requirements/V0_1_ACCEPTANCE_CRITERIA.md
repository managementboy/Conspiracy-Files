...
| CF-V01-E06 | Each D1–D6 body is readable in full through the mechanism proven safe for static/runtime text while normal inventory/container behavior remains intact. | blocked pending a named spike | After T7, test name/body/page persistence and reader behavior before/after save/reload for all six preferred interaction types. | Full authored candidate text and correct title display without leaking hidden data, replacing vanilla Lua or corrupting normal item behavior. | blocked — T7 / Issue #8 | T7 |
...
| CF-V01-E10 | Death and reload do not duplicate, reorder or erase canonical discoveries or expose partially staged state; if death-recap work is implemented for v0.1, its deterministic output uses only the last valid canonical knowledge and never reveals hidden truth. | live Project Zomboid test | Exercise death before/after discoveries, save/reload around death handling, abrupt interruption at supported lifecycle points and no-AI operation; if a recap is present, also verify its deterministic output. | Canonical knowledge remains internally consistent with no duplication, reordering, erasure or partial staged commit; a recap is not required for base-slice acceptance, but any implemented recap is deterministic, knowledge-bounded and independent of pending AI/network work. | awaiting implementation | None assigned; exact lifecycle probe/owner is not yet named. |
