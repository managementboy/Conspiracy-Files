...
Louisville, the world map shows no numbers. Notebook and case text fall back
...
- **Text:** `labelForBuilding` (`:76-81`), `nearest` (`:51-70`) and `describe`
  ...
  DiscoveryLog.
- **A second copy:** `Notebook.lua:689-1122` holds a hot-load copy of all
  three address files, generated by `tools/sync_notebook_bundles.py:28`. Any
  change must be re-synced there.
- **Help** still says house numbers "are not available yet"
  (`Notebook.lua:86`).
...
A save that has already frozen a Muldraugh address book may have notebook
...
- One code path, which matches P4-R63 and P4-R77 (a rules change means a new
  game).
- In an existing save, notebook entries that quote a number can **stop
...
already written in a notebook changes.
...
3. Walk to one numbered house and confirm the notebook or organiser shows the
   same address.
...
| Notebook bundle drifts from `AddressMap.lua` | The hot-load copy shows the old behaviour. | Verified risk (`Notebook.lua:689-1122`); resync with `tools/sync_notebook_bundles.py` in the same change. |
| Help text is wrong | It says numbers are "not available yet" (`Notebook.lua:86`). | Verified; update the text with this work. |
...
| 6 | `AddressMap` reads the shipped file; header check; save nothing; implement the section 5 answer. Resync the Notebook bundle. Update Help. | M |
