...
| "Cases past ten" | **Built (2026-09-17).** The archive of P4-R111: `MAX_CASES=16` is now the store's cap, with `MAX_ACTIVE=4` live and `MAX_FULL_ARCHIVED=4` archived cases keeping their rows; older archived cases keep their ids, questions and answers only. | `SuccessiveCases.lua`; `RetiredCase.shrink`; `docs/design/CASE_RETIREMENT.md` | Fifteen cases in a save can be steered. A stubbed case can still be answered about and still steers. |
