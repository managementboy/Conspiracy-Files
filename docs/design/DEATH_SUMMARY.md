...
The death recap is an emotional payoff, not a network-dependent transaction.
...
- **Alt-F4 during death flow:** on next valid load, use persisted death/recap state if the game/save semantics allow it; exact PZ lifecycle must be probed before final spec.
