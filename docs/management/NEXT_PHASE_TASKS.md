...
| NP-08 | Keep a multi-case record understandable | Separate cases in the record, retain evidence after item loss, link only learned information and make map markers identifiable across cases. Avoid spoilers and duplicate pins. | Offline projection/UI fixtures after NP-07 contract; Terra Low worker. Native usability check before acceptance. |
| NP-09 | Complete player-facing navigation and help | Explain numbering, writing tools, pending marks and availability in ordinary language. Verify one case record keybind and non-debug access; address remaining ambiguous destination references. Development diagnostics remain debug-only. | Scoped Terra/Luna work on established rules; ordinary-play check depends on NP-04/08. Do not remove debug guards before mechanism acceptance. |
