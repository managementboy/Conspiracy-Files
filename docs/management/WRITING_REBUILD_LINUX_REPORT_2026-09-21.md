...
| Knowledge and source voice | **PASS** — discovery orders covered exhaustively; source text on the native page; missing essential evidence blocks completion. One clause, *optional evidence is not required for the local answer*, holds by implication rather than direct assertion: `essential_links` fixes the three anchors as the essential set and completion keys on that set | `test/discovery_order.lua` (120 cases in 1,476 orders), `test/story_family_contract.lua`, `test/case_completion_state.lua`, `test/essential_links.lua` |
...
| Placement and recovery | **Partial** — interruption recovery PASS (four interruption points, five designs through save and reload) and kind variety measured (five container kinds, four rooms). **Downgraded by my own audit:** the variety measurement is ONE case, its rooms are kitchen/hall/bathroom/livingroom with **no bedroom** although the handoff asks for bedroom storage by name, and nothing covers repeat fresh-house starts, deferred scans, fill/exploration order, moved or dismantled furniture, replacement containers at the same coordinates, or extra vanilla loot | `20260921T061655-gate4-placement-variety.txt`, `20260920T224635-map-placement.txt` |
