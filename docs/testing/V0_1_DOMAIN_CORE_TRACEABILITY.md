...
- `Validator.lua` enforces the T1-safe scalar/table subset, no cycles/aliases/metatables, depth 64, the v0.1 schema, exact marked-subject exclusivity, reverse-reachable fallback authority and the 500 KB estimate ceiling; the current `Journal.lua` additionally owns exact per-kind construction and causal replay against Evidence and confirmation order.
...
|---|---|---|
| CF-V01-P04 | `CF-V01-P04 D1/D2 introduction paths expose only ordinary-text leads` | Fresh D1-first and loss-authorized D2-first states; single introduction; actual Evidence context; no objective, map marker, completion or solved state. |
...
| CF-V01-P18 | `CF-V01-P18 staged recursive validation rejects unsafe states and preserves last-known-good`; `CF-V01-P18 exact journal language rejects swapped confirmations and impossible per-kind fields`; `integrity model accepts public roots and rejects systematic root and journal mutations without persistence writes` | Invalid structural values, schema/static-ID errors, depth 64/65, repeated staged-failure preservation, public-root differential reconstruction, every canonical root field, swapped discovery/confirmation associations, every journal kind's required/forbidden fields, duplicates, eligibility and impossible causal histories. |
