...
- `LoadGridsquare` only deduplicates exact D1–D6 target wake-ups. `OnGameStart`
  prepares one save-scoped token per document and queues catch-up work. World
...
- The T4 sequence is `pending` → durably staged `placing` → detached
  ...
  container rescan → durably staged `placed`. One existing token repairs stale
  intent; two or more tokens enter sticky `conflict`; no automatic deletion or
  restamp exists.
- Placement outcome and physical availability are separate. Bounded production
  observation covers the player inventory, accepted placement container,
  nearby floor/corpse squares and the occupied vehicle. One observed token is
  `available`; incomplete zero coverage is `unknown`; a supplied conclusive
...
8. optional no-token `untracked` operation and sticky duplicate conflict;
...
after-add, after-verify, after-ledger-commit and duplicate-token paths. For every
valid path, perform a real target stream-out/in and three save/reloads, then
repeat target callbacks. Assert exactly one D1 token and `placed`; duplicates
...
accepted containers. Verify fallback state never suppresses D3–D6.

### CF-V01-E03 — fallback

Exercise: D1 success then discovery; D1 target unload; zero only in the original
container; incomplete `unknown`; no-token `untracked`; duplicate `conflict`;
...
allows. Save/reload at every stage and inspect both token and canonical
...
Also run the adapter seam with no token and require `untracked` without loss of
discovery/journal truth. Corpse reload remains unclaimed until this matrix
completes.
...
byte with `dead-air-r1`. Confirm normal inventory/container transfer still
